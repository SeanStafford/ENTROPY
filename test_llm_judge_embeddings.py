"""
Test LLM judge on embeddings retriever.
"""
...
# Direct import for embeddings to avoid __init__.py issues
import importlib.util
spec = importlib.util.spec_from_file_location(
    "embedding_retrieval",
    PROJECT_ROOT / "entropy/contexts/retrieval/embedding_retrieval.py"
)
emb_module = importlib.util.module_from_spec(spec)
spec.loader.exec_module(emb_module)
EmbeddingDocumentStore = emb_module.EmbeddingDocumentStore

DATA_PATH = Path(os.getenv("DATA_PROCESSED_PATH"))

# Initialize
logger.info("Initializing LLM judge and Embeddings retriever...")
judge = LLMJudge(model="gpt-4o-mini")
emb_store = EmbeddingDocumentStore.load(DATA_PATH / "embedding_store_20stocks.pkl")

# Same test queries as BM25
test_queries = [
    {
...
# Run evaluation
results = evaluate_with_llm_judge(emb_store, test_queries, judge, k=5)
